...
namespace COM.SingNo.XNLLib.XNL
{
    ...
    /*内容 上一条 下一条 导航标签
    内容页可不设置contentid,表示当前内容  其它类型页不设置会报错
     ...
     * 1.先得到所有栏目列表
         ...
         * 3.统计栏目模型表字段
         * 4.建联合查询sql
        * <XNL:ContentNav contentid="" channelIndex="" channelName="" upLevel="显示上几级栏目的内容列表，0为本级，1为父栏目，以此类推" topLevel="显示从首页向下某个级别栏目的内容列表，0代表网站首页，1代表一级栏目，以此类推" scope="Self(只显示本栏目下的所有内容)|Children(只显示下一极子栏目下的所有内容)|SelfAndChildren(显示本级栏目及下一极子栏目下的所有内容)|Descendant(显示所有级别的子栏目下的所有内容)|All(显示全部内容) " 
...
where	  	  	获取内容列表的条件判断
        * <contentNav.prev noexist="没有上一条了">
     * <contentNav.prev/>
         * <contentNav.next noexist="没有下一条了">
        * </XNL:ContentNav>
     */
    public class ContentNav:IXNLTag<WebContext>
    {
        ...
        public string main(XNLTagStruct tagStruct, WebContext XNLPage)
        {
            ...
            XNLParam contentIdParam;
            if(!labelParams.TryGetValue("contentid",out contentIdParam)){
                if(XNLPage.contentId==0){
                    throw (new Exception("未设置contentId参数!"));
                }
                contentIdParam=new XNLParam("contentid",XNLPage.contentId,XNLType.Int32);
            }
            int contentId=Convert.ToInt32(contentIdParam.value);
            //得到所有内容参数列表
            Dictionary<string, List<string>> allVarList = XNLUtils.getContentNavVarList(labelContentStr); //得到所有字段列表
            XNLParam _tmpParam;
            ...
            {
                //得到相关内容
                DataTable tag_dt = DataHelper.ExecuteDataTable("select distinct ct_name from sn_contenttags where ct_contentid=" +contentId.ToString() + " and ct_nodeid=" +channelid.ToString());
                string tags = "";
            ...
            MatchCollection itemsColls = Regex.Matches(labelContentStr, @"<contentnav\.([a-z]+)[\s]*[^<>]*>(.*)</contentnav\.\1>", XNLBaseCommon.XNL_RegexOptions);
            //替换数据行
            Match prevMatch=null;
            Match nextMatch=null;
            MatchCollection prevVarColls=null;
            MatchCollection nextVarColls=null;
            ...
            foreach (Match match in itemsColls)
            {
                switch (match.Groups[1].Value.ToLower())
                {
                    case "prev": //上一条 
                        prevMatch = match;
                        prevVarColls = Regex.Matches(match.Groups[2].Value, RegxpEngineCommon.RegexStr_XNLAttriableVariable, XNLBaseCommon.XNL_RegexOptions);
                        break;
                    case "next": //下一条
                        nextMatch = match;
                        nextVarColls = Regex.Matches(match.Groups[2].Value, RegxpEngineCommon.RegexStr_XNLAttriableVariable, XNLBaseCommon.XNL_RegexOptions);
                        break;
                }
            }
            if (modelColls.Count > 0)
            {
                ...
                //if (labelParams.TryGetValue("tag", out _tmpParam)) tag= _tmpParam.theValue.ToString();
                ...
                DataTable prevItemDt=null;
                DataTable nextItemDt = null;
                if (sql.Length > 0)
                {
                    prevItemDt = DataHelper.ExecuteDataTable("select top 1 * from (" + sql + ") where id>" + contentId.ToString()+" order by id asc");
                    nextItemDt = DataHelper.ExecuteDataTable("select top 1 * from (" + sql + ") where id<" + contentId.ToString());

                    if (prevItemDt.Rows.Count > 0 || nextItemDt.Rows.Count > 0)
                    {
                        int titleWord = 0;
                        ...
                        if (prevItemDt.Rows.Count > 0)
                        {
                            _nodeid = Convert.ToInt32(prevItemDt.Rows[0]["nodeid"]);
                        ...
                        if (prevItemDt.Rows.Count > 0 && prevMatch != null)
                        {
                            contentSb.Replace(prevMatch.Groups[0].Value, getItemContent(tmpNode, prevItemDt.Rows[0], labelParams, prevMatch.Groups[0].Value, prevVarColls, XNLPage, project, titleWord, ellipsis));
                        }
                        else
                        {
                            getNotExitsItemStr(contentSb, prevMatch, XNLPage, "prev");
                        }
                        if (nextItemDt.Rows.Count > 0)
                        {
                            _nodeid = Convert.ToInt32(nextItemDt.Rows[0]["nodeid"]);
                            if (tmpNode == null || tmpNode.nodeID != _nodeid)
                            {
                                if (!nodeColls.TryGetValue(_nodeid, out tmpNode))
                                {
                        ...
                        if (nextItemDt.Rows.Count > 0 && nextMatch != null)
                        {
                            contentSb.Replace(nextMatch.Groups[0].Value, getItemContent(tmpNode, nextItemDt.Rows[0], labelParams, nextMatch.Groups[0].Value, nextVarColls, XNLPage, project, titleWord, ellipsis));
                        }
                        else
                        {
                            getNotExitsItemStr(contentSb, nextMatch, XNLPage, "next");
                        }
        ...
        private string getItemContent(Cchannel channel, DataRow row, Dictionary<string, XNLParam> attParams, String contentStr, MatchCollection varColls, WebContext XNLPage, TemplateProject project, int titleWord, string ellipsis)
        {
            if (varColls.Count > 0)
            {
                ...
                foreach (Match match in varColls)
                {
                    ...
                    XNLParam param = null;
                    if (!(attribStr.TrimStart('@').StartsWith("contentnav.") || attParams.TryGetValue(attribStr, out param))) continue;
                    if (attribStr.Substring(0, 1) == "@")
                    {
                        ...
                        {
                            if ((lowTrueAttr.StartsWith("contentnav.") || attParams.ContainsKey(lowTrueAttr)) && !attrList.Contains(_attrStr)) attrList.Add(_attrStr);
                        }
                        else //添加
                        {
                    ...
                    else  //可以替换
                    {
                        ...
                        {
                            if (attribStr.StartsWith("contentnav."))
                            {
        ...
        private void getNotExitsItemStr(StringBuilder contentSb, Match itemMatch, XNLContext XNLPage, string itemType)
        {
            if (itemMatch != null && itemMatch.Groups.Count>0)
                {
                    Dictionary<string, XNLParam> p_labelParams = RegxpEngineCommon.getXNLParams(itemMatch.Groups[0].Value, XNLPage);
                    XNLParam noExitPrevParam;
                    if (p_labelParams.TryGetValue("noexist", out noExitPrevParam))
                    {
                        contentSb.Replace(itemMatch.Groups[0].Value, noExitPrevParam.value.ToString());
                    }
                    else
                    {
                        contentSb.Replace(itemMatch.Groups[0].Value, (itemType == "prev" ? "没有上一条了" : "没有下一条了"));
                    }
                }
        }
        */
