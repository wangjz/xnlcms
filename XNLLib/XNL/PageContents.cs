...
{
    public class PageContents : IXNLTag<WebContext>
    {
...
where	  	  	获取内容列表的条件判断
        ...
        */
        public string main(XNLTagStruct tagStruct,WebContext XNLPage)
        {
            ...
            //得到所有内容参数列表
            Dictionary<string, List<string>> allVarList = XNLUtils.getPageContentsVarList(labelContentStr); //得到所有字段列表
            MatchCollection labelColls = RegxpEngineCommon.matchsXNLTagByName(labelContentStr, "xnl", "pagecontents");
            ...
            if (labelParams.TryGetValue("isrelated", out _tmpParam)) //是否显示相关内容
            {
                ...
                {
                    //得到相关内容
                    DataTable tag_dt = DataHelper.ExecuteDataTable("select distinct ct_name from sn_contenttags where ct_contentid=" + XNLPage.contentId.ToString() + " and ct_nodeid=" + XNLPage.curChannel.nodeID.ToString());
                    string tags = "";
            ...
            //得到所有模型表
            int itemCount = 0;
            labelParams.Add("pagecontents.count", new XNLParam(itemCount));
            string nodeSql = "";
            int isGroup = 0;
            ...
            int SpearStepNum = 1;
            MatchCollection itemsColls = Regex.Matches(labelContentStr, @"<pagecontents\.([a-z]+)[\s]*[^<>]*>(.*)</pagecontents\.\1>", XNLBaseCommon.XNL_RegexOptions);
            //替换数据行
            ...
            foreach (Match match in itemsColls)
            {
                switch (match.Groups[1].Value.ToLower())
                {
                    case "item":
                        itemMatch = match;
                        contentSb.Replace(match.Value, "◇singno_pagecontents[head]◎◇singno_pagecontents[item]◎◇singno_pagecontents[foot]◎");
            ...
            if (modelColls.Count > 0)
            {
                ...
                if (labelParams.TryGetValue("tag", out _tmpParam)) tag = _tmpParam.value.ToString();
                if (labelParams.TryGetValue("where", out _tmpParam)) where = _tmpParam.value.ToString();
                string []sql_arr= getSql(totalnum, order.ToLower(), modelColls, allVarList, isGroup, group_ids, isImage, isFile, isTop, isRecommend, isHot, isColor, tag, where);
                DataTable dt=null;
                if (sql_arr.Length > 0)
                {
                    string sql = sql_arr[0];
                    labelParams.Add("countsql", new XNLParam(sql_arr[1]));
                    DbConnection dbConn = DataHelper.CreateConnection();
                    dbConn.Open();
                    DbCommand cmd = DataHelper.GetSqlStringCommand(sql);
                    cmd.Connection = dbConn;
                     dt = DataHelper.GetDataTableBySqlWithPage(cmd, labelParams);// DataHelper.ExecuteDataTable(cmd);
                    dbConn.Close();
                }
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    labelParams["pagecontents.count"].value = dt.Rows.Count - startNum;
                    ...
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        if (j % 2 == 0)
                        {
                            if (itemMatch != null)
                            {
                                contentSb.Replace("◇singno_pagecontents[item]◎", getItemContent(tmpNode, row, labelParams, (labelColls.Count > 0 ? RegxpEngineCommon.enabledNestedXNLTag(itemMatch.Groups[2].Value, labelColls) : itemMatch.Groups[2].Value), itemVarColls, j + 1, XNLPage, itemCount.ToString(),titleWord,ellipsis) + "◇singno_pagecontents[item]◎");
                            }
                        }
                        else
                        {
                            if (alterMatch != null)
                            {
                                contentSb.Replace("◇singno_pagecontents[item]◎", getItemContent(tmpNode, row, labelParams, (labelColls.Count > 0 ? RegxpEngineCommon.enabledNestedXNLTag(alterMatch.Groups[2].Value, labelColls) : alterMatch.Groups[2].Value), alterItemVarColls, j + 1, XNLPage, itemCount.ToString(), titleWord, ellipsis) + "◇singno_pagecontents[item]◎");
                            }
                            else
                            {
                               if (itemMatch != null)
                         ...
                         if (separMatch != null)
                         {
                            ...
                            {
                                contentSb.Replace("◇singno_pagecontents[item]◎", getItemContent(tmpNode, row, labelParams, (labelColls.Count > 0 ? RegxpEngineCommon.enabledNestedXNLTag(separMatch.Groups[2].Value, labelColls) : separMatch.Groups[2].Value), spearItemVarColls, j + 1, XNLPage, itemCount.ToString(), titleWord, ellipsis) + "◇singno_pagecontents[item]◎");
                            }
                         }
                         j++;
                    }
                    ...
                    if (headMatch != null)
                    {
                        contentSb.Replace("◇singno_pagecontents[head]◎", (labelColls.Count > 0 ? RegxpEngineCommon.replaceAttribleVariable(labelParams, RegxpEngineCommon.enabledNestedXNLTag(headMatch.Groups[2].Value, labelColls)) : RegxpEngineCommon.replaceAttribleVariable(labelParams, headMatch.Groups[2].Value)));
                    }
                    else { contentSb.Replace("◇singno_pagecontents[head]◎", ""); }
                    if (footMatch != null)
                    {
                        contentSb.Replace("◇singno_pagecontents[foot]◎", (labelColls.Count > 0 ? RegxpEngineCommon.replaceAttribleVariable(labelParams, RegxpEngineCommon.enabledNestedXNLTag(footMatch.Groups[2].Value, labelColls)) : RegxpEngineCommon.replaceAttribleVariable(labelParams, footMatch.Groups[2].Value)));
                    }
                    else { contentSb.Replace("◇singno_pagecontents[foot]◎", ""); }
                    contentSb.Replace("◇singno_pagecontents[item]◎", "");
        ...
        {
            if (grouptype > 0 && groups.Length == 0) return new string[]{};
            int i = 0;
            string tmpSql = "";
            string countSql = "";
            ...
            countSql ="select count(id) from ("+ tmpSql+")";
            tmpSql += " " + orderStr + " ";
            return new string[]{tmpSql,countSql};
        }
        ...
        private string setEmptyContent(StringBuilder contentSb, Dictionary<string, XNLParam> labelParams, MatchCollection contentsColls, Match itemMatch, Match alterMatch, Match headMatch, Match footMatch, Match spearMatch, Match emptyMatch)
        {
            if (headMatch != null)
            {
                contentSb.Replace("◇singno_pagecontents[head]◎", (contentsColls.Count > 0 ? RegxpEngineCommon.replaceAttribleVariable(labelParams, RegxpEngineCommon.enabledNestedXNLTag(headMatch.Groups[2].Value, contentsColls)) : RegxpEngineCommon.replaceAttribleVariable(labelParams, headMatch.Groups[2].Value)));
            }
            else
            {
                contentSb.Replace("◇singno_pagecontents[head]◎", "");
            }
            if (footMatch != null)
            {
                contentSb.Replace("◇singno_pagecontents[head]◎", (contentsColls.Count > 0 ? RegxpEngineCommon.replaceAttribleVariable(labelParams, RegxpEngineCommon.enabledNestedXNLTag(footMatch.Groups[2].Value, contentsColls)) : RegxpEngineCommon.replaceAttribleVariable(labelParams, footMatch.Groups[2].Value)));
            }
            else
            {
                contentSb.Replace("◇singno_pagecontents[foot]◎", "");
            }
            if (emptyMatch != null)
            {
                contentSb.Replace("◇singno_pagecontents[head]◎", (contentsColls.Count > 0 ? RegxpEngineCommon.replaceAttribleVariable(labelParams, RegxpEngineCommon.enabledNestedXNLTag(emptyMatch.Groups[2].Value, contentsColls)) : RegxpEngineCommon.replaceAttribleVariable(labelParams, emptyMatch.Groups[2].Value)));
            }
            else
            {
                contentSb.Replace("◇singno_pagecontents[item]◎", "");
        ...
        private string getItemContent(Cchannel channel, DataRow row, Dictionary<string, XNLParam> attParams, String contentStr, MatchCollection varColls, int itemid, WebContext XNLPage, string countStr, int titleWord, string ellipsis)
        {
            if (varColls.Count > 0)
            {
                string contentPath = (channel.templateProjectColls[XNLPage.templateProjectId].getContentUrl(channel.nodeID, row["id"].ToString()).Replace("@", channel.siteWebPath)).Replace("//", "/");
                ...
                foreach (Match match in varColls)
                {
                    ...
                    XNLParam param = null;
                    if (!(attribStr.TrimStart('@').StartsWith("pagecontents.") || attParams.TryGetValue(attribStr, out param))) continue;
                    if (attribStr.Substring(0, 1) == "@")
                    {
                        ...
                        {
                            if ((lowTrueAttr.StartsWith("pagecontents.") || attParams.ContainsKey(lowTrueAttr)) && !attrList.Contains(_attrStr)) attrList.Add(_attrStr);
                        }
                        else //添加
                        {
                    ...
                    else  //可以替换
                    {
                        ...
                        {
                            if (attribStr.StartsWith("pagecontents."))
                            {
