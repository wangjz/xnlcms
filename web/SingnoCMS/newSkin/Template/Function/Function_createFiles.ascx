...
<table class="forms" cellpadding="0" cellspacing="0">
	<tbody>
		...
		<tr id="createTrow" class="hide">
			<td valign="top">选择单页：</td>
			<td>
				<select name="pageList" size="20"  multiple="multiple"  id="pageList">
					<xnl:repeater  projectID="{&projectID}" sqlcommand="select TemplateID,TemplateName from sn_template where TemplateStyle&gt;=3 and SiteID=@siteid and TemplateProjectID=@projectID">
						<attrs>
							<attr name="siteid" type="int">
								<CMS.manage:common action="getsiteidbysn"></CMS.manage:common>
							</attr>
                ...
                <div class="grayBg"><input type="button" name="startBtn" id="startBtn" value="开始生成" onclick="toCreate()"/></div>
			</td>
		</tr>
...
function toCreate()
{
	...
	createProgressBar();
	Sio.ajax({url:'Function_CreateAction.aspx?projectID={&projectID}&pagestyle=3&action=create&r='+Math.random(),method:'post',complete:createLoadComplate,data:{pageList:nodesStr}});
}
///////////////////////////////////////////////////////////////////
var inter=-1;
var allPageCount=1;
...
var state="init";
Sio.ajax({url:'Function_CreateAction.aspx?projectID={&projectID}&pagestyle=3&action=create&debug=true&r='+Math.random(),method:'post',complete:initLoadComplate});
...
{
	pb.setValue(progressNum);
}
function progress()
{
	Sio.ajax({url:'Function_CreateAction.aspx?projectID={&projectID}&pagestyle=3&action=create&debug=true&r='+Math.random(),method:'post',complete:progressLoadComplate});
}
...
function toCancle()
{
	clearInterval(inter);
	inter=-1;
	Sio.ajax({url:'Function_CreateAction.aspx?pagestyle=3&action=cancle&r='+Math.random(),complete:function(e){ alert(e);}});
}
</script>
</else>
</ifItem></xnl:if>
