...
<table class="tables">
	<thead>
		<tr>
			<td width="2%"></td>
			<td width="83%">白名单标题</td>
			<td width="15%" align="center">操作</td>
		</tr>
	</thead>
	 <CMS.Manage:MSystemConfig  action="viewwhite">
	<tbody id="ctrl_cmd">
...
function del(ip)
{
	Sio.ajax({url:'Config_SystemAction.aspx?action=delwhite',method:'post',complete:complete,data:{ip:ip}});
	//new Sio.Ajax("Config_SystemAction.aspx?action=delwhite",{parameters:{ip:ip},onComplete:complete});
}
function deleteSelect()
{
	var obj=document.getElementsByName("classListItem");
	var ipStr="";
	Sio.each(obj,function(i,e){
		if(e.checked){
			ipStr += (i>0?",":"")+e.value;
		}
	});
	Sio.ajax({url:'Config_SystemAction.aspx?action=delwhite',method:'post',complete:complete,data:{ip:ipStr}});
	//new Sio.Ajax("Config_SystemAction.aspx?action=delwhite",{parameters:{ip:ipStr},onComplete:complete});
}
function update(ip)
{
	...
	if(newIp==ip)
	{
		document.location.href="config_manageWhiteList.aspx";
		return;
	}
	Sio.ajax({url:'Config_SystemAction.aspx?action=updatewhite',method:'post',complete:complete,data:{ip:ip,newip:newIp}});
	//new Sio.Ajax("Config_SystemAction.aspx?action=updatewhite",{parameters:{ip:ip,newip:newIp},onComplete:complete});
}
