...
EDiaryEditor = {
	...
	initView: function (sParentID) {
		...
		var _toolBarInfo = [
			...
			{l: 531, t: 10, w: 24, h: 23, a: "插入翻页", n: "page"},
	...
	initControl: function (bWordMode, sContent) {
		...
		function viewDialog(el, Act) {
			...
			if(window._isIE) {
				...
				if(Act == "img") {
					window.showModalDialog("editor/img.htm" + location.search, window, "dialogWidth: 400px; dialogHeight: 200px; help: no; scroll: no; status: no");
				}
				if(Act == "link") {
	...
	runCMD: function (CMD, sValue) {
		...
		if(CMD == "" || CMD == null)return;
			...
			var html = rng.text + "[SITESERVER_PAGE]";
			oRTE.focus();
			if(window._isIE) {
				var oSelection = _this.iframe.contentWindow.document.selection.createRange();
