...
<div id="case">
	<ul><xnl:repeater sqlcommand="select ArrChildID from sn_nodes where nodeid={[channel.id]}"><itemTemplate><xnl:repeater sqlcommand="select title,id,Summary,nodeid,ImageUrl,adddate from sn_u_article  where nodeid in ({$ArrChildID})">
		<itemTemplate>
<li><xnl:channel id="{$nodeid}">
			<a href="case/{@channel.indexname}/{$id}.html"><img src="{$ImageUrl}" alt="" /><span>{$title}</span></a><br />
			{$summary}			<div>{[toshortdate('{$adddate}')]} <a href="case/{@channel.indexname}/{$id}.html">查看</a></div>
		</li></xnl:channel>
</itemTemplate>
		</xnl:repeater>
