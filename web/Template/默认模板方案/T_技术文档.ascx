...
<div id="articleList">
	<ul><xnl:repeater nodeid="{[channel.id]}" sqlcommand="select  id,title,Summary,adddate from sn_u_article where nodeid=52 order by id desc"><itemtemplate>
		<li>
			<div class="fl"><img src="{[site.path]}images/pic1.jpg" alt="" /></div>
			<div class="fr">
				<div class="title"><a href="{[site.path]}support/{[channel.indexname]}/{$id}.html">{$title}</a><span>{[toshortdate('{$adddate}')]}</span></div>
				<div class="txt">{$Summary}</div>
				<div class="enter"><a href="{[site.path]}support/{[channel.indexname]}/{$id}.html">查看</a></div>
			</div>
		</li>
        </itemtemplate>
        </xnl:repeater>
