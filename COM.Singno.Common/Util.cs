...
namespace COM.SingNo.Common
{
   public class Util
   {
       ...
       {
             return Regex.IsMatch(value, @"^[+-]?\d*[.]?\d*$",Util.Matchoptions);
       }
       public static bool IsInt(string value)
       {
            return Regex.IsMatch(value, @"^[+-]?\d*$", Util.Matchoptions);
       }
       ...
       public static bool IsDate(string value)
        {
            return Regex.IsMatch(value, @"^(?ni:(?=\d)((?'year'((1[6-9])|([2-9]\d))\d\d)(?'sep'[/.-])(?'month'0?[1-9]|1[012])\2(?'day'((?<!(\2((0?[2469])|11)\2))31)|(?<!\2(0?2)\2)(29|30)|((?<=((1[6-9]|[2-9]\d)(0[48]|[2468][048]|[13579][26])|(16|[2468][048]|[3579][26])00)\2\3\2)29)|((0?[1-9])|(1\d)|(2[0-8])))(?:(?=\x20\d)\x20|$))?((?<time>((0?[1-9]|1[012])(:[0-5]\d){0,2}(\x20[AP]M))|([01]\d|2[0-3])(:[0-5]\d){1,2}))?)$", Util.Matchoptions);
        }
       public static bool IsEMail(string value)
        {
            return Regex.IsMatch(value, @"\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*", Util.Matchoptions);
        }
       public static bool IsFlash(string value)
        {
            return Regex.IsMatch(value, @"\.swf$", Util.Matchoptions);
        }
       public static bool IsIdCard(string value)
        {
            return Regex.IsMatch(value, @"\d{17}[x]|\d{18}|\d{15}|\d{14}[x]$", Util.Matchoptions);
        }
       public static bool IsImage(string value)
        {
            return Regex.IsMatch(value, @"\.gif$|\.jpg$|\.bmp$|\.ico$|\.png$", Util.Matchoptions);
        }
       public static bool IsFileType(string value,string fileType)
        {
            return Regex.IsMatch(value, @"\." + fileType + "$", Util.Matchoptions);
        }
       public static bool IsUrl(string value)
        {
            return Regex.IsMatch(value, @"^http\:\/\/[a-zA-Z0-9\-\._]+(\.[a-zA-Z0-9\-\._]+){2,}(\/?)([a-zA-Z0-9\-\.\?\,\'\/\\\+&%\$#_]*)?$", Util.Matchoptions);
        }
       public static bool IsZipNo(string value)
        {
            return Regex.IsMatch(value, @"^\d{6}$", Util.Matchoptions);
        }
       /// <summary>
       /// 是否自然数
       /// </summary>
       ...
       public static bool IsGreateThanZero(string value)
        {
            return Regex.IsMatch(value, "^([1-9]|d*", Util.Matchoptions);
        }
       public static bool IsHanZi(string value)
        {
            return Regex.IsMatch(value, "[\u4e00-\u9fa5]", Util.Matchoptions);
        }
       public static bool IsTelNo(string value)
        {
            return Regex.IsMatch(value, @"(\(\d{3}\)|\d{3}-)?\d{8}", Util.Matchoptions);
        }
       public static bool IsCellPhotoNo(string value)
        {
            return Regex.IsMatch(value, @"/^[+]{0,1}(\d){1,3}[   ]?([-]?((\d)|[   ]){1,12})+$/", Util.Matchoptions);
        } 
       public static bool IsMatch(string input,string pattern)
       {
           Regex regex = new Regex(pattern,Util.Matchoptions);
       ...
       {
           input = Util.decodeHtml(input);
           input = Util.decodeXNL(input);
           return input.Replace("&quot;","\"");
       }
       public static string encodeHtmlAndXnl(string input)
       {
           input = Util.encodeHtml(input);
           input = Util.encodeXNL(input);
           return input;
       }
...
/*日期函数完*/
       ...
       {
           return Common.CHS2PinYin.Convert(chs, separator, initialCap);
       }
       /// <summary>
       /// 将指定中文字符串转换为拼音缩写形式。
       /// </summary>
       ...
       {
           string returnStr="";
           string pinyinStr=Common.CHS2PinYin.Convert(chs, "***|||***", initialCap);
       ...
       public static string getPcFontList()
       {
          ...
          FontFamily[] fontFamilies= installedFontCollection.Families;
           ...
           for (int j = 0; j < count; ++j)
           {
               ...
               if (j != count-1)familyList = familyList + ",";
           }
           return familyList;
       }
